...
int main(int argc, char *argv[]) {
    ...
    if (access(topology.c_str(), F_OK) == 0) {
        ...
        if (topo_file.is_open()) {
            ...
            while (std::getline(topo_file, line)) {
                if (line.empty() || line[0] == '#') continue;
                
                std::istringstream iss(line);
                int node_id;
                int read_bw, write_bw;
                int read_lat, write_lat;
                int node_capacity;
                
                if (iss >> node_id >> read_bw >> write_bw >> read_lat >> write_lat >> node_capacity) {
                    auto expander = new CXLMemExpander(read_bw, write_bw, read_lat, write_lat, node_id, node_capacity);
                    
                    SPDLOG_INFO("Created expander {}: read_bw={}, write_bw={}, read_lat={}, write_lat={}, capacity={}",
                             node_id, read_bw, write_bw, read_lat, write_lat, node_capacity);
                    
                    controller->insert_end_point(expander);
                }
            }
            
            // Now construct the topology
            std::string newick = "(1,2)";  // Simple topology with two expanders
            controller->construct_topo(newick);
            topo_file.close();
        }
    } else {
